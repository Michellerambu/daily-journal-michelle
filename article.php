...
if (isset($_POST['tambah'])) {
    ...
    echo "<script>alert('Article berhasil ditambahkan!'); window.location='admin.php?page=article';</script>";
}
...
if (isset($_POST['update'])) {
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] == 0) {
        // Hapus gambar lama
        $sql_old = "SELECT gambar FROM article WHERE id = ?";
        ...
        if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
            ...
            $stmt->bind_param("sssi", $judul, $isi, $target_file, $id);
        }
    } else {
    ...
    echo "<script>alert('Article berhasil diupdate!'); window.location='admin.php?page=article';</script>";
}

// Proses hapus article
if (isset($_GET['hapus'])) {
    $id = $_GET['hapus'];
    ...
    echo "<script>alert('Article berhasil dihapus!'); window.location='admin.php?page=article';</script>";
}
    ...
    <div class="row">
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM article ORDER BY tanggal DESC LIMIT $start, $limit";
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="admin.php?page=article&hapus=<?= $row['id'] ?>" 
                                            ...
                                            <form method="POST" enctype="multipart/form-data">
                                                <div class="modal-body">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <textarea class="form-control" name="isi" rows="4" required><?= $row['isi'] ?></textarea>
                                                    </div>
                                                    <div class="mb-3">
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                <a class="page-link" href="admin.php?page=article&halaman=<?= $page - 1 ?>">Previous</a>
            </li>
            
            <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
                <li class="page-item <?= ($page == $i) ? 'active' : '' ?>">
                    <a class="page-link" href="admin.php?page=article&halaman=<?= $i ?>"><?= $i ?></a>
                </li>
            <?php } ?>
            
            <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                <a class="page-link" href="admin.php?page=article&halaman=<?= $page + 1 ?>">Next</a>
            </li>
        </ul>
    </nav>
    <?php } ?>
